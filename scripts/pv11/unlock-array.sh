...
# Unlock ADA from the Array validator
# CIP-138: Array Type (listToArray, indexArray)
# The validator checks that array[2] == 30
...
echo -e "${CYAN}=== Unlock ADA from Array Validator (CIP-138) ===${NC}"
...
if [ -z "$SCRIPT_UTXO" ] || [ "$SCRIPT_UTXO" = "null" ]; then
  ...
  echo "Run lock-array.sh first to lock ADA at the script."
  exit 1
fi
...
# Write redeemer: {index: 2, expectedValue: 30}
redeemer_file="$txs_dir/array-redeemer.json"
cat > "$redeemer_file" << 'EOF'
{"constructor":0,"fields":[{"int":2},{"int":30}]}
EOF
...
cardano_cli conway transaction build \
  ...
  --out-file "$txs_dir/unlock-array.unsigned"

# Sign transaction
echo "Signing transaction..."
cardano_cli conway transaction sign \
  --tx-body-file "$txs_dir/unlock-array.unsigned" \
  --signing-key-file "$keys_dir/payment.skey" \
  --out-file "$txs_dir/unlock-array.signed"

# Submit transaction
echo "Submitting transaction..."
cardano_cli conway transaction submit --tx-file "$txs_dir/unlock-array.signed"

echo -e "${GREEN}Success!${NC} Unlocked ADA from Array validator."
echo "The on-chain array[2] == 30 check passed!"
